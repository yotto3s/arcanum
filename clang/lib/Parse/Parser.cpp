...
void Parser::Initialize() {
  ...
  if (getLangOpts().Borland) {
    ...
    PP.SetPoisonReason(Ident___abnormal_termination,
                       diag::err_seh___finally_block);
    PP.SetPoisonReason(Ident_AbnormalTermination,
                       diag::err_seh___finally_block);
  }
...
bool Parser::ParseTopLevelDecl(DeclGroupPtrTy &Result,
  ...
  // it.
  // Drain any ECSL annotation comments buffered before this declaration.
  // Always consume pending annotations even when ParseExternalDeclaration()
  // error-recovered to an empty DeclGroup, so they cannot leak across
  // declaration boundaries and attach to a later unrelated construct.
...
Parser::ParseExternalDeclaration(ParsedAttributes &Attrs,
  ...
  case tok::kw_static:
    ...
    if (getLangOpts().CPlusPlus && NextToken().is(tok::kw_template)) {
      Diag(ConsumeToken(), diag::warn_static_inline_explicit_inst_ignored) << 0;
      SourceLocation DeclEnd;
      return ParseDeclaration(DeclaratorContext::File, DeclEnd, Attrs,
                              DeclSpecAttrs);
    }
...
bool Parser::isDeclarationAfterDeclarator() {
  ...
  return Tok.is(tok::equal) ||  // int X()=  -> not a function def
         ...
         Tok.is(
             tok::kw___attribute) || // int X() __attr__ -> not a function def
...
Decl *Parser::ParseFunctionDefinition(ParsingDeclarator &D,
  ...
  if (getLangOpts().DelayedTemplateParsing && Tok.isNot(tok::equal) &&
    ...
    Decl *DP = Actions.HandleDeclarator(ParentScope, D, TemplateParameterLists);
    D.complete(DP);
  ...
  if (CurParsedObjCImpl && !TemplateInfo.TemplateParams &&
    ...
    D.setFunctionDefinitionKind(FunctionDefinitionKind::Definition);
    Decl *FuncDecl =
        Actions.HandleDeclarator(ParentScope, D, MultiTemplateParamsArg());
    D.complete(FuncDecl);
    D.getMutableDeclSpec().abort();
    if (FuncDecl) {
...
Parser::TryAnnotateName(CorrectionCandidateCallback *CCC,
  ...
  case NameClassificationKind::Type: {
    ...
    if (getLangOpts().ObjC && NextToken().is(tok::less) &&
      ...
      TypeResult NewType = parseObjCTypeArgsAndProtocolQualifiers(
          IdentifierLoc, Ty,
          /*consumeLastToken=*/false, NewEndLoc);
      if (NewType.isUsable())
        Ty = NewType.get();
